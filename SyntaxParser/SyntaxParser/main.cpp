...
using namespace std;
map<char, int>getnum;
char getchr[100];      //获取对应字符  
vector<string>proce;
int table[100][100];      //预测分析表  
int num = 0; int numvt = 0;    //0是‘#’，numvt表空字  //终结符
string first[100];
string follow[200];
void readin()
{
...
void print_follow()
{
	...
	for (int i = numvt + 1; i <= num; i++)
	{
		cout << "follow [" << getchr[i] << "]: ";
		for (int j = 0; j < follow[i].size(); j++)
			cout << getchr[follow[i][j] - '0'] << " ";
		cout << endl;
	}
	cout << endl;
}
